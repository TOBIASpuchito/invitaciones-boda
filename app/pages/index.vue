...
<template>
  <main class="relative min-h-screen overflow-hidden text-center">
    <div
      class="absolute inset-0 bg-[radial-gradient(circle_at_top,_rgba(222,195,193,0.55),_transparent_40%),radial-gradient(circle_at_bottom_right,_rgba(169,177,157,0.25),_transparent_32%)]"
    />

    <section
      class="relative mx-auto flex min-h-screen max-w-6xl flex-col justify-center px-6 py-16 lg:flex-row lg:items-center lg:gap-16"
    >
      <div class="max-w-2xl">
        <p class="text-sm uppercase tracking-[0.35em] text-wine/70">Boda de:</p>

        <h1
          class="mt-5 font-display text-5xl leading-tight text-cocoa sm:text-6xl"
        >
          {{ config.public.eventName }}
        </h1>

        <p class="mt-6 max-w-xl text-lg leading-8 text-stone-600">
          Escribe tu nombre y apellido para encontrar tu invitación y confirmar
          tu asistencia en un solo paso.
        </p>

        <div
          class="mt-10 rounded-[2rem] border border-white/70 bg-white/80 p-6 shadow-glow backdrop-blur sm:p-8"
        >
          <form class="space-y-4" @submit.prevent="searchInvitations">
            <div>
              <label
                for="guest-name"
                class="block text-sm font-medium text-cocoa"
              >
                Nombre y apellido
              </label>
      ...
      <div class="mt-12 w-full max-w-xl lg:mt-0">
        <div
          class="rounded-[2rem] border border-white/70 bg-white/75 p-8 shadow-glow backdrop-blur"
        >
          ...
          <div class="mt-8 grid gap-4 sm:grid-cols-2">
            ...
            <div class="rounded-2xl bg-cocoa p-5 text-white sm:col-span-2">
              <p class="mt-3 text-lg font-medium leading-7">
                Si encontramos tu nombre, te llevaremos directo a tu invitación
