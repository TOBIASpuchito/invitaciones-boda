...
<template>
  <section
    ref="sectionRef"
    class="rounded-[2rem] border border-white/70 bg-white/80 p-8 shadow-glow backdrop-blur"
  >
    <p
      data-reveal
      data-reveal-delay="0"
    ...
    <blockquote
      ...
      class="mt-6 border-l-2 border-wine/30 pl-5 space-y-2"
    >
      <p class="text-sm leading-7 text-stone-500 italic">
        Hace 12 años el destino unió a Cindy y Marcelo&hellip;
      </p>
      <p class="text-sm leading-7 text-stone-500 italic">
        y desde entonces, no han dejado de elegirse el uno al otro.
      </p>
      <p class="text-sm leading-7 text-stone-500 italic">
        Construyeron juntos una familia, un hogar y un amor que los transformó.
      </p>
      <p class="text-sm leading-7 text-stone-500 italic">
        Y ahora, dan el paso definitivo: unir sus vidas para siempre.
      </p>
      <p class="text-sm leading-7 text-stone-500 italic">
        Este no es solo su día, es la celebración de una historia que sigue
        creciendo.
      </p>
    </blockquote>
    ...
    <div data-reveal data-reveal-delay="0.18" class="mt-8 flex flex-wrap justify-center gap-3">
      ...
      <span
        class="rounded-full bg-sand px-4 py-2 text-sm font-medium text-cocoa"
      >
        {{ invitation.allowedGuests }} cupo<span
          v-if="invitation.allowedGuests !== 1"
          >s</span
        >
      </span>
      <span
    ...
    <div class="detail-widgets mt-6 grid gap-4 sm:grid-cols-2">
      ...
      <div
        ...
        class="widget widget--venue sm:col-span-2"
      >
        <div class="widget-icon">
          <svg
            viewBox="0 0 24 24"
...
.widget-ring {
  ...
  height: 7rem;
  border-radius: 50%;
  border: 1.5px solid rgba(180, 100, 110, 0.1);
